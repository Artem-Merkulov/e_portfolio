...
-- SQL скрипты создания таблиц для источника с данными о доставках

CREATE TABLE IF NOT EXISTS dds.dm_couriers (
    courier_id_dwh serial PRIMARY KEY,
    courier_id_source varchar(30) NOT NULL UNIQUE,
    name varchar NOT NULL
);

CREATE TABLE IF NOT EXISTS dds.dm_deliveries (
    delivery_id_dwh int NOT NULL PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    order_id int NOT NULL REFERENCES dds.dm_orders(id),
    order_ts timestamp NOT NULL UNiQUE,
    delivery_id_source varchar(30) NOT NULL UNIQUE,
    courier_id_source varchar(30) NOT NULL UNIQUE,
    address varchar(30) NOT NULL UNIQUE,
    delivery_ts timestamp NOT NULL UNiQUE,
    rate int NOT NULL DEFAULT 0 CHECK (rate >= 0),
    sum numeric(19, 5) NOT NULL DEFAULT 0 CHECK (sum >= 0),
    tip_sum numeric(19, 5) NOT NULL DEFAULT 0 CHECK (tip_sum >= 0)
);

CREATE TABLE IF NOT EXISTS dds.dm_delivery_restaurants (
    restaurant_id_dwh serial PRIMARY KEY,
    restaurant_id_source varchar(30) NOT NULL UNIQUE,
    name varchar NOT NULL
);


-- SQL скрипт создания таблицы фактов
